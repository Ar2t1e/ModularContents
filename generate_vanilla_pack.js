...
const outDir = "C:\\Users\\Ethan\\MCreatorWorkspaces\\modularcontents\\run\\ModularContents\\vanilla_pack\\recipes";
...
const vanillaItems = {
    "Blocks": [
        ...
        "stone_slab", "brick_block", "tnt", "bookshelf", "mossy_cobblestone", "obsidian",
        ...
        "lit_redstone_lamp", "wooden_slab", "cocoa", "sandstone_stairs", "emerald_ore",
        ...
        "hay_block", "carpet", "hardened_clay", "coal_block", "packed_ice", "standing_banner",
        "wall_banner", "red_sandstone", "red_sandstone_stairs",
        ...
        "chorus_flower", "purpur_block", "purpur_pillar", "purpur_stairs", "purpur_slab",
...
let count = 0;
for (const [category, items] of Object.entries(vanillaItems)) {
    for (const item of items) {
        const recipeId = `vanilla_${item}`;
        ...
        const recipe = {
            ...
            outputs: [
                { item: `minecraft:${item}`, count: 1, meta: 0 }
            ],
            craftingTime: 20 // 1 second for vanilla items
        };

        const outFile = path.join(outDir, `${recipeId}.json`);
        try {
            fs.writeFileSync(outFile, JSON.stringify(recipe, null, 4));
            count++;
        } catch (e) {
            console.error(`Error saving ${recipeId}: ${e}`);
        }
    }
}

console.log(`Generated ${count} vanilla recipes in ${outDir}!`);
